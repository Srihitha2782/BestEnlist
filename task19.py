...
dbse = mydb.cursor()
dbse.execute("CREATE TABLE student3 (roll_no INT(10),Reg_no INT(10),NAME VARCHAR(255), semester1 INT(25),semester2 INT(25), CGPA INT(35) ,PHONE_NUMBER INT,email_id VARCHAR(255))")
dbse = mydb.cursor()
...
import xlrd
import MySQLdb
x1_sheet = xlrd.open_workbook("students.xlsx")
x1_sheet
sheet_name =x1_sheet.sheet_names()
sheet_name
mydb = mysql.connector.connect(
    host="localhost",
    user="root",
...
cur = mydb.cursor()
for s in range(0,1):
    sheet=x1_sheet.sheet_by_index(s)
...
mydb.commit()
mycursor = mydb.cursor()
mycursor.execute("SELECT *DROM student3")
